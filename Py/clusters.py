...
from PIL imoprt Image,ImageDraw
import random
"""def pearson(v1,v2):
	#Simple Sums
	sum1=sum(v1)
	...
	#Sum Up Products
	pSums=sum([v1[i]*v2[i] for i in range(len(v1))])
...
def getdepth(clust):
	...
	return max(get depth(clust.left),getdepth(clust.right))+clust.distance
...
def kcluster(rows,distance=pearson,k=4):
	...
	lastmatches=Nonefor t in range(100):
	print 'Iteration %d' % tbestmatches=[[] for i in range(k)]
...
def scaledown(data,distance=pearson,rate=0.01):
	...
	fakedist=[[0.0 forj in range(n)] for i in range(n)]
